function fetchBreeds() {
  return fetch('https://api.thecatapi.com/v1/breeds').then(response => {
    if (!response.ok) {
      throw new Error('Oops! Something went wrong! Try reloading the page!(first fetch)');
    }
...
function fetchCatByBreed(breedId) {
  return fetch(
    ...
    .then(response => {
      ...
      if (!response.ok) {
        throw new Error('Oops! Something went wrong! Try reloading the (Помилка при отримані вмісту)');
      }
      return response.json()
    })
